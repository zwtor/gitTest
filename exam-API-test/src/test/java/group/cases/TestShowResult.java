...
public class TestShowResult {
    ...
    @Test(description = "5.新建岗位认证-考试结果隐藏接口", priority = 5)
    public void testQualificationsAdd() {
        PositionAuthentication positionAuthentication = new PositionAuthentication();
        String paperResponse = PaperBusiness.queryList("", "false");
        String paperId = (String) JSONPath.read(paperResponse, "$.list[0].id");
        String paperTitle = (String) JSONPath.read(paperResponse, "$.list[0].title");

        Boolean show_result1 = false;
        JSONArray stageJsonList1 = new JSONArray();
        JSONObject fixedPaperObject1 = positionAuthentication.createFixedPaperStageInAuthentication(1, title1, paperId, paperTitle, show_result1);
        stageJsonList1.add(fixedPaperObject1);

        String authenticationTitle1 = "Automation" + CommonData.getStringRandom(3);
        String res = positionAuthentication.createAuthentication(authenticationTitle1, stageJsonList1);
        String msg = (String) JSONPath.read(res, "$.msg");
        qualifications_exam_false_id = (String) JSONPath.read(res, "$.data");

        System.out.println("5.新建岗位认证-考试结果隐藏接口:" + "msg=" + msg + "," + "qualifications_id=" + qualifications_exam_false_id);
        Assert.assertEquals(msg, "新增计划成功！", "5.新建岗位认证-考试结果隐藏接口" + res);

        Boolean show_result2 = true;
        JSONArray stageJsonList2 = new JSONArray();
        JSONObject fixedPaperObject2 = positionAuthentication.createFixedPaperStageInAuthentication(1, title2, paperId, paperTitle, show_result2);
        stageJsonList2.add(fixedPaperObject2);

        String authenticationTitle2 = "Automation" + CommonData.getStringRandom(3);
        res = positionAuthentication.createAuthentication(authenticationTitle2, stageJsonList2);
        msg = (String) JSONPath.read(res, "$.msg");
        qualifications_exam_true_id = (String) JSONPath.read(res, "$.data");

        System.out.println("5.新建岗位认证-考试结果不隐藏接口:" + "msg=" + msg + "," + "qualifications_id=" + qualifications_exam_true_id);
        Assert.assertEquals(msg, "新增计划成功！", "5.新建岗位认证-考试结果不隐藏接口" + res);
    }


    @Test(description = "6.获取岗位认证-考试id接口", priority = 6)
    public void testQualificationsExamId() {
        String res = ShowResultBusiness.QualificationsExamId(qualifications_exam_false_id);
        qualifications_id = (String) JSONPath.read(res, "$.stage_list[0].course_mapping_list[0].id");
        System.out.println("6.获取岗位认证-考试id接口:" + "qualifications_exam_true_id=" + qualifications_id + ","
...
//            } else if (jsonObj.getString("planName").contains(exam_title2)) {
...
//    }
    ...
    @Test(description = "12.删除考试任务接口", dependsOnMethods = {"testExamAdd", "testExamAddFalse"}, priority = 12)
    ...
    @Test(description = "13.删除岗位认证接口", dependsOnMethods = "testQualificationsAdd", priority = 13)
    public void testDeleteQualifications() {
        String res = ShowResultBusiness.DeleteQualifications(qualifications_exam_false_id);
        String msg = (String) JSONPath.read(res, "$.msg");
        System.out.println("13.删除岗位认证接口:" + "msg=" + msg);
        Assert.assertEquals(msg, "能力认证删除成功", "13.删除岗位认证接口" + res);

        res = ShowResultBusiness.DeleteQualifications(qualifications_exam_true_id);
        msg = (String) JSONPath.read(res, "$.msg");
        System.out.println("13.删除岗位认证接口:" + "msg=" + msg);
        Assert.assertEquals(msg, "能力认证删除成功", "13.删除岗位认证接口" + res);
    }
