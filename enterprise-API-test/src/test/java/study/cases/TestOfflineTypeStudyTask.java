...
public class TestOfflineTypeStudyTask {
...
//		Assert.assertTrue(endTime!=null, "查看线下签到详情--结束时间，实际返回结果："+res);
//		Assert.assertEquals(processStr, "0%","查看线下签到详情--培训进度，实际返回结果："+res);
//		Assert.assertTrue(departName!=null, "查看线下签到详情--所属部门，实际返回结果："+res);
//		Assert.assertEquals(userName, user_name,"查看线下签到详情--姓名，实际返回结果："+res);
	}
...
//		Assert.assertEquals(msg, "查询线下培训不存在","线下签到不存在时，查看线下签到详情"+res);
	...
	@Test(description="编辑线下学习任务",priority=12)
	public void testEditStudyTask() {
//		String res = StudyTaskBusiness.queryInfo(id);
//		String xx_id = (String)JSONPath.read(res, "$.stageList[0].courseMappingList[0].latojaVo.id");
//		String la_id = (String)JSONPath.read(res, "$.stageList[0].courseMappingList[0].latojaVo.bizLatojaClockTimes[0].id");
//		String latojaId = (String)JSONPath.read(res, "$.stageList[0].courseMappingList[0].latojaVo.bizLatojaClockTimes[0].latojaId");
//		
//		String edit_res=StudyTaskBusiness.editStudyTask("offline task"+CommonData.getStringRandom(4), id, DateUtil.getTimeStamp(0, ""), DateUtil.getTimeStamp(3, ""),
//				xx_id,DateUtil.getRegularDateForHHMM(1), DateUtil.getRegularDateForHHMM(2), lec_id, DateUtil.getTimeStamp(1, ""), la_id, latojaId);
//		String msg = (String)JSONPath.read(edit_res, "$.msg");
//		System.out.println(edit_res);
//		Assert.assertTrue(msg.contains("修改"), "编辑线下学习任务，实际返回结果");
	}
...
//				DateUtil.getRegularDateForHHMM(0), DateUtil.getRegularDateForHHMM(2), lec_id, DateUtil.getOneHourLaterStamp(), DateUtil.getTimeStamp(2, ""));
//		String res = AppStudyBusiness.queryStudyList("unfinished");
...
//		Assert.assertTrue(departName != null, "APP端在签到时间2小时之内签到，查看线下签到详情--所属部门，实际返回结果：" + res);
	}
	@Test(description = "删除任务",priority = 22)
	public void deleteStudyTask () {
//		String res = StudyTaskBusiness.deleteStudyTask(offline_id);
//		String msg= (String)JSONPath.read(res, "$.msg");
//		Assert.assertEquals(msg, "删除学习计划成功","成功签到之后,删除考试任务，实际返回结果:"+res);
	}
