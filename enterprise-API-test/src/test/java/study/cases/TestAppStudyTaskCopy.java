...
public class TestAppStudyTaskCopy extends InitStudyAuthCourse {
	...
	String lec_name = "LKMHIUH"+CommonData.getStringRandom(6);
	@BeforeClass(description = "新增混合的学习任务")
	public void testAddMixSettledExamStudyTask() {
		LecturerListBusiness.addLecturer(lec_name,"1", "13526231231","outside" ,"", BaseBusiness.lecturerLevel, "", "jmeter");
		String lec_id = LecturerListBusiness.getIdByKeyword(lec_name);
		...
		String res = StudyTaskBusiness.addMixSettledExamStudyTask(study_name, DateUtil.getTimeStamp(0, ""),
				...
				DateUtil.getRegularDateForHHMM(2), lec_id,
				DateUtil.getTimeStamp(0, ""),
				QuestionnaireBusiness.getIdByKeyword(BaseBusiness.questionnaireName, "release"),DateUtil.getTimeStamp(2,""));
	...
	public static void main(String[] args) {
		...
		System.out.println(DateUtil.getRegularDateForHHMM(2));
	}

	@Test(description = "APP端查看混合学习列表", priority = 1)
	...
	@Test(description = "APP端加载混合学习资源", priority = 2)
	...
	@Test(description = "APP端查看混合学习详情", priority = 3)
	public void testQueryAppInfo() {
		...
		String art_name = (String) JSONPath.read(res, "$.stages[0].course_list[0].title");
		...
		String xx_name = (String) JSONPath.read(res, "$.stages[0].course_list[3].title");
		String ques_name = (String) JSONPath.read(res, "$.stages[0].course_list[4].title");
		...
		String ques_status = (String) JSONPath.read(res, "$.stages[0].course_list[4].status");
		...
		xx_Id = (String) JSONPath.read(res, "$.stages[0].course_list[3].id");
		investigatesId = (String) JSONPath.read(res, "$.stages[0].course_list[4].id");
		ques_id = (String) JSONPath.read(res, "$.stages[0].course_list[4].questionnaire_id");
		int award_score = (int) JSONPath.read(res, "$.award_score");
		
		String first_type = (String)JSONPath.read(res, "$.stages[0].course_list[0].type");
		...
		String four_type = (String)JSONPath.read(res, "$.stages[0].course_list[3].type");
		String fifth_type = (String)JSONPath.read(res, "$.stages[0].course_list[4].type");
		
		Assert.assertEquals(first_type, "article", "APP端查看混合学习详情,学习类型字段校验，实际返回结果：" + res);
		...
		Assert.assertEquals(four_type, "latoja", "APP端查看混合学习详情,学习类型字段校验，实际返回结果：" + res);
		Assert.assertEquals(fifth_type, "investigate", "APP端查看混合学习详情,学习类型字段校验，实际返回结果：" + res);
		Assert.assertEquals(award_score, 8, "APP端查看混合学习详情,奖励学分做校验，实际返回结果：" + res);
		Assert.assertEquals(art_name, articlename, "APP端查看混合学习详情，文章名称做校验，实际返回结果：" + res);
		...
		Assert.assertEquals(xx_name, "offline trainer", "APP端查看混合学习详情，线下签到名称做校验，实际返回结果：" + res);
		Assert.assertEquals(ques_name, BaseBusiness.questionnaireName, "APP端查看混合学习详情，调研名称做校验，实际返回结果：" + res);
		...
		Assert.assertEquals(type, "mix", "APP端查看混合学习详情,类型做校验，实际返回结果：" + res);
	}

	@Test(description = "App端保存学习任务中的课件进度--20%进度时", priority = 4)
	public void testsaveArtProgress() {
		String res = AppStudyBusiness.saveProgress("100", id, articleId, articleId);
		int course_progress = (int) JSONPath.read(res, "$.course_progress");
		int total_progress = (int) JSONPath.read(res, "$.total_progress");
		int study_progress = (int) JSONPath.read(res, "$.study_progress");
		Assert.assertEquals(study_progress, 25, "保存学习任务中课件的学习进度，实际返回结果：" + res);
		Assert.assertEquals(course_progress, 100, "保存学习任务中的课件进度，实际返回结果：" + res);
		Assert.assertEquals(total_progress, 20, "保存学习任务中的课件，总进度校验，实际返回结果：" + res);
	}

	@Test(description = "APP端保存学习任务中的课件进度后，查看混合学习的详情--20%进度时", priority = 5)
	...
	@Test(description = "APP端保存学习任务中的课件进度后,web端查看学习任务进行中的人员的培训进度--20%进度时", priority = 6)
	public void testQueryTrainMonitor() {
		...
		Assert.assertEquals(studyProgress, 20, "APP端保存学习任务中的课件进度后,查看学习任务进行中的人员的培训进度：" + res);
	}
	@Test(description = "APP端保存学习任务中的课件进度后,web端查看全部人员监控--20%进度时", priority = 7)
	public void testQueryUndoUserMonitors() {
		...
		Assert.assertEquals(studyProgress, 25, "APP端保存学习任务中的课件进度后,web端查看全部人员监控：" + res);
		Assert.assertEquals(process, 20, "APP端保存学习任务中的课件进度后,web端查看全部人员监控：" + res);
	}

	@Test(description = "查看APP端学习任务的课程详情", priority = 8)
	public void testQueryStudyCourseoyherInfo() {
		...
		Assert.assertEquals(lecturer_name, lecturer_name, "查看学习任务的课程详情," + res);
	}

	@Test(description = "保存APP端学习任务的课程进度", priority = 9)
	public void testSaveCourseProgress() {
		...
		Assert.assertEquals(total_progress, 40, "保存APP端学习任务的课程进度，实际返回结果：" + res);
	}

	@Test(description = "APP端对学习任务的课程进行点赞", priority = 10)
	...
	@Test(description = "APP端对学习任务的课程取消点赞", priority = 11)
	...
	@Test(description = "APP端对学习任务的课程进行评论", priority = 12)
	...
	@Test(description = "APP端保存学习任务中的课程进度后，查看混合学习的详情--40%进度时，课程进度做校验", priority = 13)
	public void testQueryAppSaveCourseInfo() {
		String res = AppStudyBusiness.queryInfo(id);
		int cour_progress = (int) JSONPath.read(res, "$.stages[0].course_list[1].progress");
		Assert.assertEquals(cour_progress, 100, "APP端保存学习任务中的课程进度后，查看混合学习的详情,课程进度做校验，实际返回结果：" + res);
	}

	@Test(description = "APP端保存学习任务中的课程进度后,web端查看学习任务进行中的人员的培训进度--40%进度时", priority = 14)
	public void testQueryTrainCourseMonitor() {
		String res = StudyTaskBusiness.queryTrainMonitor("", id);
		int studyProgress = (int) JSONPath.read(res, "$.data.voList.list[0].process");
		Assert.assertEquals(studyProgress, 40, "APP端保存学习任务中的课程进度后,查看学习任务进行中的人员的培训进度：" + res);
	}

	@Test(description = "APP端保存学习任务中的课程进度后,web端查看全部人员监控--40%进度时", priority = 15)
	public void testQueryUndoUserCourseMonitors() {
		String res = StudyTaskBusiness.queryUserMonitors(id, "true", "0", "", "0");
		int studyProgress = (int) JSONPath.read(res, "$.data.voList.list[0].studyProgress");
		int process = (int) JSONPath.read(res, "$.data.voList.list[0].process");
		int course_Progress = (int) JSONPath.read(res,
				"$.data.voList.list[0].stage_train_vo_list[0].courseInfo[1].studyProgress");
		int art_Progress = (int) JSONPath.read(res,
				"$.data.voList.list[0].stage_train_vo_list[0].courseInfo[0].studyProgress");
		Assert.assertEquals(studyProgress, 50, "APP端保存学习任务中的课程进度后,web端查看全部人员监控--学习进度" + res);
		Assert.assertEquals(process, 40, "APP端保存学习任务中的课程进度后,web端查看全部人员监控--总进度" + res);
		Assert.assertEquals(course_Progress, 100, "APP端保存学习任务中的课程进度后,web端查看全部人员监控--课程进度" + res);
		Assert.assertEquals(art_Progress, 100, "APP端保存学习任务中的课程进度后,web端查看全部人员监控--课件进度" + res);
	}

	@Test(description = "在APP端查看学习任务考试概要", priority = 16)
	...
	@Test(description = "在APP端查看学习任务的考试是否可以进行考试", priority = 17)
	...
	@Test(description = "在APP端查看学习任务考试详情", priority = 18)
	...
	@Test(description = "在APP端在学习任务提交考试", priority = 19)
	...
	@Test(description = "在APP端在学习任务查看考试结果", priority = 20)
	...
	@Test(description = "在APP端学习任务查看考试的答案解析", priority = 21)
	...
	@Test(description = "APP端查看学习任务的考试排名", priority = 22)
	...
	@Test(description = "APP端考试失败后，查看学习任务详情的考试状态", priority = 23)
	...
	@Test(description = "APP端学习任务中，试卷已经提交,不允许重复提交!", priority = 24)
	...
	@Test(description = "APP端学习任务中的考试失败--60%进度时,web端查看学习任务进行中的人员的培训进度", priority = 25)
	public void testQueryTrainExameMonitor() {
		String res = StudyTaskBusiness.queryTrainMonitor("", id);
		int studyProgress = (int) JSONPath.read(res, "$.data.voList.list[0].process");
		Assert.assertEquals(studyProgress, 60, "APP端学习任务中的考试失败--60%进度时,web端查看学习任务进行中的人员的培训进度" + res);
	}

	@Test(description = "APP端学习任务中的考试失败--60%进度时,web端查看全部人员监控", priority = 26)
	public void testQueryExamUserCourseMonitors() {
		...
		int art_Progress = (int) JSONPath.read(res,
				"$.data.voList.list[0].stage_train_vo_list[0].courseInfo[0].studyProgress");
		Assert.assertEquals(studyProgress, 50, "APP端学习任务中的考试失败--60%进度时,web端查看全部人员监控--学习进度" + res);
		Assert.assertEquals(process, 60, "APP端学习任务中的考试失败--60%进度时,web端查看全部人员监控--总进度" + res);
		Assert.assertEquals(course_Progress, 100, "APP端学习任务中的考试失败--60%进度时,web端查看全部人员监控--课程进度" + res);
		Assert.assertEquals(art_Progress, 100, "APP端学习任务中的考试失败--60%进度时,web端查看全部人员监控--课件进度" + res);
	}

	@Test(description = "APP端学习任务中的进度为60%，查看web端的学习记录", priority = 27)
	public void testQueryStudyRecord() {
		String res = AppStudyBusiness.queryStudyRecord(id);
		String art_title = (String) JSONPath.read(res, "$.data[0].courseInfo[0].title");
		int art_studyProgress = (int) JSONPath.read(res, "$.data[0].courseInfo[0].studyProgress");
		int course_studyProgress = (int) JSONPath.read(res, "$.data[0].courseInfo[1].studyProgress");
		int xx_studyProgress = (int) JSONPath.read(res, "$.data[0].courseInfo[2].studyProgress");
		String xx_title = (String) JSONPath.read(res, "$.data[0].courseInfo[2].title");
		Assert.assertEquals(art_studyProgress, 100, "APP端学习任务中的进度为60%，查看web端的学习记录" + res);
		Assert.assertEquals(xx_studyProgress, 0, "APP端学习任务中的进度为60%，查看web端的学习记录" + res);
		Assert.assertEquals(course_studyProgress, 100, "APP端学习任务中的进度为60%，查看web端的学习记录" + res);
		Assert.assertEquals(art_title, articlename, "APP端学习任务中的进度为60%，查看web端的学习记录" + res);
		Assert.assertEquals(xx_title, "offline trainer", "APP端学习任务中的进度为60%，查看web端的学习记录" + res);
	}

	@Test(description = "考试不及格后，查看学习任务的考试记录", priority = 28)
	...
	@Test(description = "APP端查看学习任务线下签到详情", priority = 29)
	public void testQueryLatojasInfo() {
		String res = AppStudyBusiness.queryLatojasInfo(id, xx_Id);
		String la_name = (String) JSONPath.read(res, "$.title");
		Long begin_time = (Long) JSONPath.read(res, "$.begin_time");
		Long end_time = (Long) JSONPath.read(res, "$.end_time");
		String address = (String) JSONPath.read(res, "$.address");
		String introduction = (String) JSONPath.read(res, "$.introduction");
		int sign_in_count = (int) JSONPath.read(res, "$.sign_in_count");
		Long clock_time = (Long) JSONPath.read(res, "$.sign_in_list[0].clock_time");
		...
		latojasId = (String) JSONPath.read(res, "$.sign_in_list[0].id");
		String status = (String) JSONPath.read(res, "$.sign_in_list[0].status");
		Assert.assertEquals(status, "signing", "APP端查看学习任务线下签到详情--签到状态，实际返回结果：" + res);
		Assert.assertEquals(address, "xi'an", "APP端查看学习任务线下签到详情--签到地址，实际返回结果：" + res);
		Assert.assertTrue(introduction.contains("this is des"), "APP端查看学习任务线下签到详情--签到描述，实际返回结果：" + res);
		Assert.assertEquals(sign_in_count, 1, "APP端查看学习任务线下签到详情--签到次数，实际返回结果：" + res);
		Assert.assertEquals(lecturer_name, lec_name, "APP端查看学习任务线下签到详情--讲师名称，实际返回结果：" + res);
		Assert.assertEquals(lecturer_describe, "jmeter", "APP端查看学习任务线下签到详情--讲师描述，实际返回结果：" + res);
		Assert.assertTrue(begin_time != null, "APP端查看学习任务线下签到详情--结束时间，实际返回结果：" + res);
		Assert.assertTrue(clock_time != null, "APP端查看学习任务线下签到详情--签到时间，实际返回结果：" + res);
		Assert.assertTrue(end_time != null, "APP端查看学习任务线下签到详情--开始时间，实际返回结果：" + res);
		Assert.assertEquals(la_name, "offline trainer", "APP端查看学习任务线下签到详情--name，实际返回结果：" + res);
	}

	@Test(description = "APP端在签到时间后进行签到，显示迟到", priority = 30)
	...
	@Test(description = "APP端签到后迟到，查看学习任务线下签到详情", priority = 31)
	...
	@Test(description = "APP端签到后迟到,web端查看学习任务进行中的人员的培训进度--80%进度时", priority = 32)
	public void testQueryOfflineSignTrainMonitor() {
		String res = StudyTaskBusiness.queryTrainMonitor("", id);
		int studyProgress = (int) JSONPath.read(res, "$.data.voList.list[0].process");
		Assert.assertEquals(studyProgress, 80, "APP端签到后迟到,web端查看学习任务进行中的人员的培训进度--80%进度时：" + res);
	}

	@Test(description = "APP端签到后迟到，在学习任务详情查看线下签到的状态和进度", priority = 33)
	...
	@Test(description = "APP端签到后迟到,web端查看全部人员监控--80%进度时", priority = 34)
	public void testQueryOfflineSignUserMonitors() {
		String res = StudyTaskBusiness.queryUserMonitors(id, "true", "0", "", "0");
		int studyProgress = (int) JSONPath.read(res, "$.data.voList.list[0].studyProgress");
		int process = (int) JSONPath.read(res, "$.data.voList.list[0].process");
		Assert.assertEquals(studyProgress, 75, "APP端签到后迟到,web端查看全部人员监控--80%进度时：" + res);
		Assert.assertEquals(process, 80, "APP端签到后迟到,web端查看全部人员监控--80%进度时：" + res);
	}

	String la_id = "";
	
	@Test(description = "APP端签到后迟到，Web端查看数据监控的线下培训列表", priority = 35)
	...
	@Test(description = "APP端签到后迟到，查看线下签到详情", priority = 36)
	public void testQueryLatojaMonitorDetailList() {
		...
		Assert.assertEquals(tea_name, lec_name, "APP端签到后迟到，查看线下签到详情--线下培训的讲师，实际返回结果：" + res);
	...
	@Test(description = "APP端签到后迟到,web端查询学习任务列表--80%进度时", priority = 37)
	...
	@Test(description = "APP端查看学习任务下的调研详情", priority = 38)
	...
	@Test(description = "匿名，可查看统计结果，提交问卷", priority = 39)
	...
	@Test(description = "APP端完成学习任务后,查看已完成的学习列表", priority = 40)
	public void testQueryAPPFinishedStudyList() {
		...
		Assert.assertEquals(type, "mix", "APP端完成学习任务后,查看已完成的学习列表,类型做校验，实际返回结果：" + res);
	}

	@Test(description = "APP端完成学习任务后，查看学习任务详情", priority = 41)
	...
	@Test(description = "APP端完成学习任务后,web端查看全部人员监控--100%进度时", priority = 42)
	...
	@Test(description = "APP端完成学习任务后,web端查询学习任务列表", priority = 43)
	...
	@Test (description="APP端完成学习任务后.查看问卷调研的统计数据",priority=44)
	public void testQueryLatojaMoitor() {
		...
		Assert.assertEquals(title_a,BaseBusiness.questionnaireName,"APP端完成学习任务后,查看问卷调研的统计数据，调研的名称实际返回结果"+res);
	}
	
	@Test(description="APP端完成学习任务后，查看问卷调研详情，参数含有userid的 ",priority=45)
	...
	@Test(description="APP端完成学习任务后，查看问卷调研明细",priority=46)
	...
	@Test(description="APP端完成学习任务后，查看调研统计数据",priority=47)
	public void testQueryInvestigatesStatisticsInfo() {
		...
		Assert.assertEquals(title, study_name,"APP端完成学习任务后，查看调研统计数据,实际返回结果："+res);
	}
	@Test(description = "清除所有导出数据",priority = 48)
	public void deleteAllRecord() {
		String res = PaperExportBusiness.deleteAllRecord();
		Assert.assertTrue(res.contains("OK"),"清除所有导出数据,实际返回结果："+res);
	}
	@Test(description = "导出线下签到的数据",priority = 49)
	...
	@Test(description = "导出问卷调研统计的数据",priority = 50)
	...
	@Test(description = "导出问卷明细的数据",priority = 51)
	...
	@Test(description="导出考试结果--通过用户名查看",priority=52)
	...
	@Test(description="导出考试结果--无搜索条件的情况",priority=53)
	...
	@Test(description="导出学员作答明细--通过用户名查看",priority=54)
	...
	@Test(description="导出学员作答明细--无搜索条件的情况",priority=55)
	...
	@Test(description="导出试题作答明细--通过用户名查看",priority=56)
	...
	@Test(description="导出试题作答明细--无搜索条件的情况",priority=57)
	...
	@Test(description="导出试题分析--全部试题类型的情况",priority=58)
	...
	@Test(description="导出试题分析--单选题类型的情况",priority=59)
	...
	@Test(description="导出试题分析--多选试题类型的情况",priority=60)
	...
	@Test(description="导出试题分析--判断试题类型的情况",priority=61)
